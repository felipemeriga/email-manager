...
async fn main() -> Result<()> {
    ...
    info!("Configuration loaded: {}:{}", settings.server.host, settings.server.port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            // Email endpoints
            .route("/emails/recent", web::get().to(email_handlers::get_recent_emails))
            .route("/emails/today", web::get().to(email_handlers::get_today_emails))
            .route("/emails/by-date/{date}", web::get().to(email_handlers::get_emails_by_date))
            .route("/emails/search", web::post().to(email_handlers::search_emails))
            .route("/emails/{id}/read", web::post().to(email_handlers::mark_as_read))
            .route("/emails/{id}/unread", web::post().to(email_handlers::mark_as_unread))
            .route("/emails/{id}", web::delete().to(email_handlers::delete_email))
            .route("/emails/bulk-delete", web::post().to(email_handlers::bulk_delete))
    })
